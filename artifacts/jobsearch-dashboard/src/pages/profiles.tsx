      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-4">
            <Button 
              className="w-full h-12 rounded-xl shadow-md border border-primary/20 bg-primary/10 text-primary hover:bg-primary/20 hover:-translate-y-0.5 transition-all"
              variant="outline"
              ...
              {listLoading ? (
                ...
                profiles.map(name => {
                  ...
                  return (
                    <div 
                      key={name}
                      onClick={() => { setSelectedName(name); setIsCreating(false); }}
...
function ProfileEditor({ mode, profileName, isActive, onSuccess }: { mode: 'create' | 'edit', profileName?: string, isActive?: boolean, onSuccess?: (name: string) => void }) {
  ...
  const { fields, append, remove } = useFieldArray({ control: form.control, name: "queries" });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            return checked
                              ...
                              : field.onChange(field.value?.filter((val) => val !== source))
                          }}
                        />
                      </FormControl>
                ...
                <div className="space-y-3">
                  {fields.map((item, index) => (
                    <div key={item.id} className="flex items-start gap-3 p-1">
                      ...
                      <Button type="button" variant="ghost" size="icon" onClick={() => remove(index)} className="h-11 w-11 shrink-0 text-muted-foreground hover:text-destructive hover:bg-destructive/10">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    </div>
                  ))}
          ...
          <CardFooter className="bg-muted/20 border-t border-border/50 px-6 py-4">
            <Button type="submit" disabled={isCreating || isUpdating} className="ml-auto rounded-xl px-8 h-12 shadow-lg shadow-primary/20 hover:-translate-y-0.5 transition-all font-semibold text-base">
